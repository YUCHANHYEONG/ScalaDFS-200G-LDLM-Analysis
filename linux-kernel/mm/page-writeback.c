...
EXPORT_SYMBOL(ratelimit_pages); //kiet
...
EXPORT_SYMBOL(dirty_background_ratio); //Kiet
...
EXPORT_SYMBOL(dirty_background_bytes);
...
EXPORT_SYMBOL(vm_dirty_ratio); //Kiet
...
EXPORT_SYMBOL(vm_dirty_bytes); //Kiet
...
EXPORT_SYMBOL(dirty_expire_interval); //Kiet

/*
 * Flag that makes the machine dump writes/reads and block dirtyings.
 */
int block_dump;
EXPORT_SYMBOL(block_dump); //Kiet
...
EXPORT_SYMBOL(global_dirtyable_memory);
...
EXPORT_SYMBOL(domain_dirty_limits);
...
EXPORT_SYMBOL(node_dirty_ok); //Kiet
...
EXPORT_SYMBOL(bdi_min_ratio); /*Kiet*/
...
EXPORT_SYMBOL(dirty_freerun_ceiling);
...
EXPORT_SYMBOL(wb_position_ratio);
...
EXPORT_SYMBOL(__wb_update_bandwidth);
...
EXPORT_SYMBOL(dirty_poll_interval);
...
EXPORT_SYMBOL(wb_dirty_limits);
EXPORT_SYMBOL(__tracepoint_balance_dirty_pages);
...
EXPORT_SYMBOL(dirty_throttle_leaks); //Kiet
...
void (*conn_balance_dirty_pages_ratelimited)(struct address_space *mapping);
EXPORT_SYMBOL(conn_balance_dirty_pages_ratelimited);

void my_kern_balance_dirty_pages_ratelimited(struct address_space *mapping)
{
	struct inode *inode = mapping->host;
	struct backing_dev_info *bdi = inode_to_bdi(inode);
	struct bdi_writeback *wb = NULL;
	...
	if (conn_balance_dirty_pages_ratelimited) {
		(*conn_balance_dirty_pages_ratelimited)(mapping);
		return;
	}
...
EXPORT_SYMBOL(my_kern_balance_dirty_pages_ratelimited);
...
EXPORT_SYMBOL(wb_over_bg_thresh); //Kiet
...
EXPORT_SYMBOL(do_writepages); //Kiet
