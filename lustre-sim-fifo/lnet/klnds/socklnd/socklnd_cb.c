...
#include "../../../lustre/include/calclock.h"
...
KTDEF(ksocknal_process_receive);
EXPORT_SYMBOL(ksocknal_process_receive_clock);

KTDEF(ksocknal_process_receive_wait);
EXPORT_SYMBOL(ksocknal_process_receive_wait_clock);

KTDEF(ksocknal_process_transmit);
EXPORT_SYMBOL(ksocknal_process_transmit_clock);
	...
	while (!ksocknal_data.ksnd_shuttingdown) {
		...
		if (conn) {
			...
			rc = ksocknal_process_receive(conn, rx_scratch_pgs,
						      scratch_iov);
			ktget(&localclock[1]);
			ktput(localclock, ksocknal_process_receive);
		...
		if (!list_empty(&sched->kss_tx_conns)) {
			...
			ktget(&localclock[1]);
			ktput(localclock, ksocknal_process_transmit);
		...
		ktget(&localclock[1]);
		ktput(localclock, ksocknal_process_receive_wait);
	}
