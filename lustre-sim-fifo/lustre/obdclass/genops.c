...
KTDEF(obd_get_request_slot_wait);
EXPORT_SYMBOL(obd_get_request_slot_wait_clock);
...
int obd_get_request_slot_internal(struct client_obd *cli)
{
	struct obd_request_slot_waiter	 orsw;
	int				 rc;
	...
	rc = l_wait_event_abortable(orsw.orsw_waitq,
				    ...
				    orsw.orsw_signaled);
	ktget(&localclock[1]);
	ktput(localclock, obd_get_request_slot_wait);
...
EXPORT_SYMBOL(obd_get_request_slot_internal);
...
KTDEF(claim_mod_rpc_function);
EXPORT_SYMBOL(claim_mod_rpc_function_clock);
				  ...
				  unsigned int mode, int flags, void *key)
{
	...
	if (avail) {
		...
		ret = 0;
	ktget(&localclock[1]);
	ktput(localclock, claim_mod_rpc_function);
	return ret;
}

KTDEF(obd_get_mod_rpc_slot_wait);
EXPORT_SYMBOL(obd_get_mod_rpc_slot_wait_clock);
...
__u16 obd_get_mod_rpc_slot_internal(struct client_obd *cli, __u32 opc)
{
	struct mod_waiter wait = {
		.cli = cli,
		.close_req = (opc == MDS_CLOSE),
	...
	while (wait.woken == false) {
		...
		wait_woken(&wait.wqe, TASK_UNINTERRUPTIBLE,
			   MAX_SCHEDULE_TIMEOUT); 
		ktget(&localclock[1]);
		ktput(localclock, obd_get_mod_rpc_slot_wait);
		spin_lock_irq(&cli->cl_mod_rpcs_waitq.lock);
	}
	__remove_wait_queue(&cli->cl_mod_rpcs_waitq, &wait.wqe);
	max = cli->cl_max_mod_rpcs_in_flight;
	// pr_info("Max flight = %d \n", max);
	lprocfs_oh_tally(&cli->cl_mod_rpcs_hist,
			 cli->cl_mod_rpcs_in_flight);
	/* find a free tag */
...
EXPORT_SYMBOL(obd_get_mod_rpc_slot_internal);
