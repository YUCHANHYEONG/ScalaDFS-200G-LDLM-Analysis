 ...
 * osc_set_async_flags().
 */
int osc_io_submit_internal(const struct lu_env *env, const struct cl_io_slice *ios,
...
KTDEF(osc_io_submit);
EXPORT_SYMBOL(osc_io_submit_clock);
		  ...
		  enum cl_req_type crt, struct cl_2queue *queue)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = osc_io_submit_internal(env, ios, crt, queue);
	ktget(&localclock[1]);
	ktput(localclock, osc_io_submit);
	return ret;
}
...
KTDEF(osc_page_cache_add);
EXPORT_SYMBOL(osc_page_cache_add_clock);
			...
			cl_commit_cbt cb)
{
	...
	while (qin->pl_nr > 0) {
		...
		if (list_empty(&oap->oap_pending_item)) {
			...
			// ktget(&localclock[1]);
			// ktput(localclock, osc_page_cache_add);
			if (result != 0)
				break;
		}
...
KTDEF(osc_cache_writeback_range);
EXPORT_SYMBOL(osc_cache_writeback_range_clock);
int fsync_osc_cache_writeback_range(const struct lu_env *env, struct osc_object *obj,
			      pgoff_t start, pgoff_t end, int hp, int discard);

static int osc_io_fsync_start_internal(const struct lu_env *env,
			      const struct cl_io_slice *slice)
{
	struct cl_io       *io  = slice->cis_io;
...
KTDEF(osc_io_fsync_start);
EXPORT_SYMBOL(osc_io_fsync_start_clock);
			      ...
			      const struct cl_io_slice *slice)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = osc_io_fsync_start_internal(env, slice);
	ktget(&localclock[1]);
	ktput(localclock, osc_io_fsync_start);
	return ret;
}

KTDEF(osc_io_fsync_end);
EXPORT_SYMBOL(osc_io_fsync_end_clock);

KTDEF(osc_io_fsync_end_wait_for_completion);
EXPORT_SYMBOL(osc_io_fsync_end_wait_for_completion_clock);

KTDEF(osc_cache_wait_range);
EXPORT_SYMBOL(osc_cache_wait_range_clock);

void osc_io_fsync_end_internal(const struct lu_env *env,
		      const struct cl_io_slice *slice)
{
	...
	if (fio->fi_mode == CL_FSYNC_LOCAL) {
		...
		ktget(&localclock[1]);
		ktput(localclock, osc_io_fsync_end_wait_for_completion);
	}
		      ...
		      const struct cl_io_slice *slice)
{
	ktime_t localclock[2];
	ktget(&localclock[0]);
	osc_io_fsync_end_internal(env, slice);
	ktget(&localclock[1]);
	ktput(localclock, osc_io_fsync_end);
}
...
KTDEF(osc_io_end);
EXPORT_SYMBOL(osc_io_end_clock);
...
KTDEF(osc_lru_reserve);
EXPORT_SYMBOL(osc_lru_reserve_clock);
		       ...
		       loff_t pos, size_t bytes)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_lru_reserve);
