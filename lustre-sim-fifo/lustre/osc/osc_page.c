...
KTDEF(osc_queue_async_io);
EXPORT_SYMBOL(osc_queue_async_io_clock);
		       ...
		       struct cl_io *io, cl_commit_cbt cb)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_queue_async_io);
	if (result != 0)
 ...
 * max_pages_per_rpc.
 */
static long osc_lru_reclaim_internal(struct client_obd *cli, unsigned long npages)
{
	struct lu_env *env;
	struct cl_client_cache *cache = cli->cl_cache;
...
KTDEF(osc_lru_reclaim);
EXPORT_SYMBOL(osc_lru_reclaim_clock);
...
static long osc_lru_reclaim(struct client_obd *cli, unsigned long npages)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = osc_lru_reclaim_internal(cli, npages);
	ktget(&localclock[1]);
	ktput(localclock, osc_lru_reclaim);
	return ret;
}
...
KTDEF(osc_lru_reserve_wait);
EXPORT_SYMBOL(osc_lru_reserve_wait_clock);
...
again:
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_lru_reserve_wait);
