...
KTDEF(ptlrpc_queue_wait);
EXPORT_SYMBOL(ptlrpc_queue_wait_clock);

KTDEF(ptlrpc_queue_wait_mdt);
EXPORT_SYMBOL(ptlrpc_queue_wait_mdt_clock);
		     ...
		     struct lustre_handle *lockh, int async)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_queue_wait);
...
KTDEF(ptlrpc_get_mod_rpc_slot);
EXPORT_SYMBOL(ptlrpc_get_mod_rpc_slot_clock);

KTDEF(obd_get_request_slot);
EXPORT_SYMBOL(obd_get_request_slot_clock);
		     ...
		     struct lustre_handle *lockh, int async)
{
	...
	if (einfo->ei_mod_slot)
		ptlrpc_get_mod_rpc_slot_internal(req);

	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_get_mod_rpc_slot);
	...
	if (need_req_slot) {
		ktget(&localclock[0]);
		rc = obd_get_request_slot_internal(&req->rq_import->imp_obd->u.cli);
		ktget(&localclock[1]);
		ktput(localclock, obd_get_request_slot);
		if (rc) {
			if (einfo->ei_mod_slot)
				ptlrpc_put_mod_rpc_slot(req);
			failed_lock_cleanup(ns, lock, einfo->ei_mode);
	...
	ktget(&localclock[0]);
	rc = ptlrpc_queue_wait_mdt(req);
	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_queue_wait_mdt);
...
EXPORT_SYMBOL(ldlm_cli_enqueue_mdt);
