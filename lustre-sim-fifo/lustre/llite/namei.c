...
KTDEF(md_intent_lock);
EXPORT_SYMBOL(md_intent_lock_clock);
...
inherit:
	...
	// ktget(&localclock[1]);
	// ktput(localclock, md_intent_lock);
...
static struct dentry *ll_lookup_it_internal(struct inode *parent, struct dentry *dentry,
				   struct lookup_intent *it,
				   void **secctx, __u32 *secctxlen,
				   int *secctxslot,
				   struct pcc_create_attach *pca,
...
inherit:
	...
	ktget(&localclock[0]);
	rc = md_intent_lock_mdt(ll_i2mdexp(parent), op_data, it, &req,
			    &ll_md_blocking_ast, 0);
	ktget(&localclock[1]);
	ktput(localclock, md_intent_lock);
	/* If the MDS allows the client to chgrp (CFS_SETGRP_PERM), but the
...
KTDEF(ll_lookup_it);
EXPORT_SYMBOL(ll_lookup_it_clock);

/*
 * For cached negative dentry and new dentry, handle lookup/create/open
 * together.
 */
static int ll_atomic_open_internal(struct inode *dir, struct dentry *dentry,
			  ...
			  umode_t mode ll_last_arg)
{
	...
	ktget(&localclock[0]);
	de = ll_lookup_it_internal(dir, dentry, it, &secctx, &secctxlen, &secctxslot,
			  &pca, encrypt, &encctx, &encctxlen);
	// de = ll_lookup_it(dir, dentry, it, &secctx, &secctxlen, &secctxslot,
	// 		  &pca, encrypt, &encctx, &encctxlen);
	ktget(&localclock[1]);
	ktput(localclock, ll_lookup_it);
	if (IS_ERR(de))
		rc = PTR_ERR(de);
	else if (de != NULL)
		dentry = de;
...
KTDEF(ll_atomic_open);
EXPORT_SYMBOL(ll_atomic_open_clock);
			  ...
			  umode_t mode ll_last_arg)
{
	...
	ktget(&localclock[0]);
#ifdef FMODE_CREATED /* added in Linux v4.18-rc1-20-g73a09dd */
	ret = ll_atomic_open_internal(dir, dentry, file, open_flags, mode);
#else
	ret = ll_atomic_open_internal(dir, dentry, file, open_flags, mode, opened);
#endif
	ktget(&localclock[1]);
	ktput(localclock, ll_atomic_open);
	return ret;
}
