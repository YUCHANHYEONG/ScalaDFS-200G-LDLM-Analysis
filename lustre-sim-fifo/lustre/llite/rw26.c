 ...
 * final lustre_iput(), umount, or memory pressure induced icache shrinking).
 *
...
KTDEF(ll_direct_rw_pages);

static ssize_t
ll_direct_IO_impl_internal(struct kiocb *iocb, struct iov_iter *iter, int rw)
{
	...
	while (iov_iter_count(iter)) {
		...
		result = ll_direct_rw_pages(env, io, count,
					    rw, inode, sdio);
		ktget(&localclock[1]);
		ktput(localclock, ll_direct_rw_pages);
...
ll_direct_IO_impl(struct kiocb *iocb, struct iov_iter *iter, int rw){
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = ll_direct_IO_impl_internal(iocb, iter, rw);
	ktget(&localclock[1]);
	ktput(localclock, ll_direct_IO_impl);
	return ret;
}
...
KTDEF(grab_cache_page_nowait);
EXPORT_SYMBOL(grab_cache_page_nowait_clock);

KTDEF(cl_page_find);
EXPORT_SYMBOL(cl_page_find_clock);
...
again:
	...
	// vmpage = grab_cache_page_nowait(mapping, index);
	ktget(&localclock[0]);
	vmpage = lustre_grab_cache_page_nowait(mapping, index);
	ktget(&localclock[1]);
	ktput(localclock, grab_cache_page_nowait);
...
#else
	.set_page_dirty		= __lustre_set_page_dirty_nobuffers,
#endif
#ifdef HAVE_INVALIDATE_FOLIO
