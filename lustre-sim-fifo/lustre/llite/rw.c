	...
	io->ci_ignore_layout = 1;
		...
		if (!IS_ERR(page)) {
			...
			if (result != 0) {
				 ...
				 */
				if (!PageError(vmpage)) {
					lustre_redirty_page_for_writepage(wbc, vmpage);
					result = 0;
					redirtied = true;
				}
			}
...
KTDEF(cl_sync_file_range);
EXPORT_SYMBOL(cl_sync_file_range_clock);
...
int fsync_ll_writepages(struct address_space *mapping, struct writeback_control *wbc)
{
	...
	result = fsync_cl_sync_file_range(inode, start, end, mode, 1);
	//result = cl_sync_file_range(inode, start, end, mode, 1);
	ktget(&localclock[1]);
	ktput(localclock, cl_sync_file_range);
...
int ll_writepages_internal(struct address_space *mapping, struct writeback_control *wbc)
{
	struct inode *inode = mapping->host;
	loff_t start;
	loff_t end;
...
KTDEF(ll_writepages);
EXPORT_SYMBOL(ll_writepages_clock);
...
int ll_writepages(struct address_space *mapping, struct writeback_control *wbc){
	...
	if (wbc->sync_mode == WB_SYNC_ALL) {
		//printk("[%s] pid=%d\n", __func__, current->pid);
		ret = fsync_ll_writepages(mapping, wbc);
	}

	ktget(&localclock[0]);
	ret = ll_writepages_internal(mapping, wbc);
	ktget(&localclock[1]);
	ktput(localclock, ll_writepages);
	return ret;
}
