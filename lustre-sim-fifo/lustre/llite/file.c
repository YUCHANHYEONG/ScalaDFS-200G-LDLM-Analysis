 ...
 */
int ll_file_release_internal(struct inode *inode, struct file *file)
{
	struct ll_file_data *fd;
...
int ll_file_release(struct inode *inode, struct file *file)
{
	...
	ktget(&localclock[0]);
	ret = ll_file_release_internal(inode, file);
	ktget(&localclock[1]);
	ktput(localclock, ll_file_release);
...
KTDEF(md_intent_lock_ll_open);
EXPORT_SYMBOL(md_intent_lock_ll_open_clock);

static int ll_intent_file_open_internal(struct dentry *de, void *lmm, int lmmsize,
...
retry:
	...
	ktget(&localclock[1]);
	ktput(localclock, md_intent_lock_ll_open);
	kfree(name);
	ll_finish_md_op_data(op_data);
	if (rc == -ESTALE) {
...
KTDEF(ll_intent_file_open);
KTDEC(ll_intent_file_open_clock);
				...
				struct lookup_intent *itp)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = ll_intent_file_open_internal(de, lmm, lmmsize, itp);
	ktget(&localclock[1]);
	ktput(localclock, ll_intent_file_open);
	return ret;
}
...
KTDEF(cl_io_loop);
KTDEF(cl_sync_io_wait_recycle);

static ssize_t
ll_file_io_generic_internal(const struct lu_env *env, struct vvp_io_args *args,
...
restart:
	...
	if (cl_io_rw_init(env, io, iot, *ppos, per_bytes) == 0) {
		...
		ktget(&localclock[0]);
		rc = cl_io_loop_profile(env, io); //kiet
		ktget(&localclock[1]);
		ktput(localclock, cl_io_loop);
	...
	if (io->ci_dio_aio && !is_aio) {
		...
		ktget(&localclock[1]);
		ktput(localclock, cl_sync_io_wait_recycle);
		if (rc2 < 0)
			rc = rc2;
	}
		   ...
		   loff_t *ppos, size_t count)
{
	...
	ssize_t ret;
	ktget(&localclock[0]);
	ret = ll_file_io_generic_internal(env, args, file, iot, ppos, count);
	ktget(&localclock[1]);
	ktput(localclock, ll_file_io_generic);
	return ret;
}
 ...
 */
static ssize_t ll_file_write_iter_internal(struct kiocb *iocb, struct iov_iter *from)
{
	struct vvp_io_args *args;
	struct lu_env *env;
...
static ssize_t ll_file_write_iter(struct kiocb *iocb, struct iov_iter *from){
	...
	ktget(&localclock[1]);
	ktput(localclock, ll_file_write_iter);
	return ret;
}
...
KTDEF(cl_sync_loop);
EXPORT_SYMBOL(cl_sync_loop_clock);
 ...
 */
int fsync_cl_sync_file_range(struct inode *inode, loff_t start, loff_t end,
		       enum cl_fsync_mode mode, int ignore_layout)
{
	...
	if (cl_io_init(env, io, CIT_FSYNC, io->ci_obj) == 0){
		...
		result = fsync_sync_cl_io_loop(env, io);
		//result = sync_cl_io_loop(env, io);
		//printk("[%s] pid=%d ych_2\n", __func__, current->pid);
		ktget(&localclock[1]);
		ktput(localclock, cl_sync_loop);
	}
	else
		result = io->ci_result;
	if (result == 0)
 ...
 */
extern int fsync_filemap_write_and_wait_range(struct address_space *mapping,
...
int ll_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	rc = fsync_filemap_write_and_wait_range(inode->i_mapping, start, end);
...
int ll_getattr_dentry_internal(struct dentry *de, struct kstat *stat, u32 request_mask,
		      ...
		      unsigned int flags, bool foreign)
{
	...
	ktget(&localclock[0]);
	ret = ll_getattr_dentry_internal(de, stat, request_mask, flags, foreign);
	ktget(&localclock[1]);
	ktput(localclock, ll_getattr_dentry);
	return ret;
}
