...
{
	struct ptlrpcd_ctl *pc;
	// pr_info("Application add request\n");
	...
	application_set_add_new_req(req, set);
}
EXPORT_SYMBOL(application_add_req);
...
KTDEF(ptlrpcd_check);
EXPORT_SYMBOL(ptlrpcd_check_clock);
...
static int ptlrpcd_check(struct lu_env *env, struct ptlrpcd_ctl *pc)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, ptlrpcd_check);
...
KTDEF(wait_woken);
EXPORT_SYMBOL(wait_woken_clock);
