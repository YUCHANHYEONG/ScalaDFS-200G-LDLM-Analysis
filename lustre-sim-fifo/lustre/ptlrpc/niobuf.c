...
static int k_ptl_send_buf(struct lnet_handle_md *mdh, void *base, int len,
			enum lnet_ack_req ack, struct ptlrpc_cb_id *cbid,
			struct lnet_nid *self, struct lnet_processid *peer_id,
			int portal, __u64 xid, unsigned int offset,
			struct lnet_handle_md *bulk_cookie)
{
	...
	// pr_info("Sending %d bytes to portal %d, xid %lld, offset %u, peer id %s\n",
	//        len, portal, xid, offset, libcfs_idstr(peer_id));
...
int ptlrpc_register_bulk(struct ptlrpc_request *req)
{
	...
	for (posted_md = 0; posted_md < desc->bd_md_count;
		...
		ptlrpc_fill_bulk_md(&md, desc, posted_md);
		// pr_info("posted_md = %d, desc->bd_md_count = %d, md.length = %d, md.start = %p\n",	posted_md, desc->bd_md_count, md.length, md.start);
	...
	// pr_info("Setup %u bulk %s buffers: %u pages %u bytes, mbits x%#llx-%#llx, portal %u\n",
	//        desc->bd_refs,
	//        ptlrpc_is_bulk_op_get(desc->bd_type) ? "get-source" : "put-sink",
	//        desc->bd_iov_count, desc->bd_nob,
	//        desc->bd_last_mbits, req->rq_mbits, desc->bd_portal);
	...
	// 	pr_info("bd_vec[0] = %p\n", desc->bd_vec);
	// else
	// 	pr_info("bd_vec[0] = NULL\n");

	RETURN(0);
}

int ptlrpc_register_bulk_md(struct ptlrpc_request *req)
{
	struct ptlrpc_bulk_desc *desc = req->rq_bulk;
	struct lnet_processid peer;
	int rc = 0;
	...
	for (posted_md = 0; posted_md < desc->bd_md_count;
		...
		md.options = PTLRPC_MD_OPTIONS |
			     (ptlrpc_is_bulk_op_get(desc->bd_type) ?
			      LNET_MD_OP_GET : LNET_MD_OP_PUT);
		ptlrpc_fill_bulk_md_multiple(&md, desc, posted_md);
		// pr_info("posted_md = %d, desc->bd_md_count = %d, md.length = %d, md.start = %p\n",	posted_md, desc->bd_md_count, md.length, md.start);
...
KTDEF(ptl_send_buf);
EXPORT_SYMBOL(ptl_send_buf_clock);
 ...
 * Returns 0 on success or error code.
 */
int ptl_send_rpc_combined(struct ptlrpc_request *request, int noreply)
{
	int rc;
	__u32 opc;
	int mpflag = 0;
	...
	/* bulk register should be done after wrap_request() */
	if (request->rq_bulk != NULL) {
		rc = ptlrpc_register_bulk_md(request);
		if (rc != 0)
			GOTO(cleanup_bulk, rc);
		/*
		 * All the mds in the request will have the same cpt
	...
	DEBUG_REQ(D_INFO, request, "send flags=%x",
		  lustre_msg_get_flags(request->rq_reqmsg));
	ktget(&localclock[0]);
	rc = k_ptl_send_buf(&request->rq_req_md_h,
			  request->rq_reqbuf, request->rq_reqdata_len,
			  LNET_NOACK_REQ, &request->rq_req_cbid,
			  NULL,
			  &connection->c_peer,
...
EXPORT_SYMBOL(ptl_send_rpc_combined);
