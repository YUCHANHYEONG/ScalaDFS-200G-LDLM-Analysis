...
void reply_in_callback(struct lnet_event *ev)
{
	...
	// if (atomic_read(&req->application) == 3)
	// 	return;
...
out_wake:
	...
	// if (atomic_cmpxchg(&req->application, 1, 2) == 1)
	// {
	...
	// 	spin_unlock(&req->rq_lock);
	// 	pr_info("Start to check \n");
	// 	application_check_req(&temp_env, req->rq_set);
	// 	pr_info("Check done \n");
...
void client_bulk_callback_combined(struct lnet_event *ev)
{
	struct ptlrpc_cb_id     *cbid = ev->md_user_ptr;
	struct ptlrpc_bulk_desc *desc = cbid->cbid_arg;
	struct ptlrpc_request *req = cbid->cbid_arg;
