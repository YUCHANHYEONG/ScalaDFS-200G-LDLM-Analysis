...
KTDEF(mdc_enqueue_send);
EXPORT_SYMBOL(mdc_enqueue_send_clock);
...
static int mdc_enqueue_send_internal(const struct lu_env *env, struct obd_export *exp,
			    ...
			    struct ldlm_enqueue_info *einfo, int async)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = mdc_enqueue_send_internal(env, exp, res_id, flags, policy, lvb, upcall, cookie, einfo, async);
	ktget(&localclock[1]);
	ktput(localclock, mdc_enqueue_send);
	return ret;
}
 ...
 */
static int mdc_lock_enqueue_internal(const struct lu_env *env,
			    const struct cl_lock_slice *slice,
...
KTDEF(mdc_lock_enqueue);
EXPORT_SYMBOL(mdc_lock_enqueue_clock);
			    ...
			    struct cl_io *unused, struct cl_sync_io *anchor)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = mdc_lock_enqueue_internal(env, slice, unused, anchor);
	ktget(&localclock[1]);
	ktput(localclock, mdc_lock_enqueue);
	return ret;
}
