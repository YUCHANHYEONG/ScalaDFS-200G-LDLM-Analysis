 ...
 */
static int mdc_enqueue_base_internal(struct obd_export *exp,
			    struct ldlm_enqueue_info *einfo,
...
KTDEF(ldlm_cli_enqueue_mdt);
EXPORT_SYMBOL(ldlm_cli_enqueue_mdt_clock);

static int mdc_enqueue_base_internal_mdt(struct obd_export *exp,
			    struct ldlm_enqueue_info *einfo,
			    const union ldlm_policy_data *policy,
			    struct lookup_intent *it,
			    struct md_op_data *op_data,
...
resend:
	...
	ktget(&localclock[0]);
	rc = ldlm_cli_enqueue_mdt(exp, &req, einfo, &res_id, policy, &flags, NULL,
			      0, lvb_type, lockh, 0);
	ktget(&localclock[1]);
	ktput(localclock, ldlm_cli_enqueue_mdt);
...
KTDEF(mdc_enqueue_base);
EXPORT_SYMBOL(mdc_enqueue_base_clock);
			    ...
			    __u64 extra_lock_flags)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = mdc_enqueue_base_internal(exp, einfo, policy, it, op_data, lockh, extra_lock_flags);
	ktget(&localclock[1]);
	ktput(localclock, mdc_enqueue_base);
	return ret;
}

static int mdc_enqueue_base_mdt(struct obd_export *exp,
			    struct ldlm_enqueue_info *einfo,
			    const union ldlm_policy_data *policy,
			    struct lookup_intent *it,
			    struct md_op_data *op_data,
			    ...
			    __u64 extra_lock_flags)
{
	...
	int ret;
	// ktget(&localclock[0]);
	ret = mdc_enqueue_base_internal_mdt(exp, einfo, policy, it, op_data, lockh, extra_lock_flags);
	// ktget(&localclock[1]);
	// ktput(localclock, mdc_enqueue_base_mdt);
	return ret;
}
...
KTDEF(mdc_enqueue_base_mdt);
EXPORT_SYMBOL(mdc_enqueue_base_mdt_clock);
...
int mdc_intent_lock_internal(struct obd_export *exp, struct md_op_data *op_data,
		    struct lookup_intent *it, struct ptlrpc_request **reqp,
		    ldlm_blocking_callback cb_blocking, __u64 extra_lock_flags)
{
	struct ldlm_enqueue_info einfo = {
	...
	ktget(&localclock[0]);
	rc = mdc_enqueue_base_mdt(exp, &einfo, NULL, it, op_data, &lockh,
			      extra_lock_flags);
	ktget(&localclock[1]);
	ktput(localclock, mdc_enqueue_base_mdt);
	if (rc < 0)
