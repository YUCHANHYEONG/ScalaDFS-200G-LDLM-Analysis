...
KTDEF(md_intent_lock_lmv);
EXPORT_SYMBOL(md_intent_lock_lmv_clock);

static int lmv_intent_open_internal(struct obd_export *exp, struct md_op_data *op_data,
			   struct lookup_intent *it,
			   struct ptlrpc_request **reqp,
			   ldlm_blocking_callback cb_blocking,
			   __u64 extra_lock_flags)
...
retry:
	...
	ktget(&localclock[0]);
	rc = md_intent_lock_mdt(tgt->ltd_exp, op_data, it, reqp, cb_blocking,
			    extra_lock_flags);
	ktget(&localclock[1]);
	ktput(localclock, md_intent_lock_lmv);
	if (rc != 0)
		RETURN(rc);
	/*
	 * Nothing is found, do not access body->fid1 as it is zero and thus
...
KTDEF(lmv_intent_open);
EXPORT_SYMBOL(lmv_intent_open_clock);

int lmv_intent_lock_lmv(struct obd_export *exp, struct md_op_data *op_data,
		    struct lookup_intent *it, struct ptlrpc_request **reqp,
		    ldlm_blocking_callback cb_blocking,
		    __u64 extra_lock_flags)
{
	...
	if (it->it_op & (IT_LOOKUP | IT_GETATTR | IT_LAYOUT | IT_GETXATTR))
		...
		ktget(&localclock[0]);
		rc = lmv_intent_open_internal(exp, op_data, it, reqp, cb_blocking,
				     extra_lock_flags);
		ktget(&localclock[1]);
		ktput(localclock, lmv_intent_open);
	}
	else
		LBUG();
...
KTDEF(lmv_intent_lock);
EXPORT_SYMBOL(lmv_intent_lock_clock);

int lmv_intent_lock_internal(struct obd_export *exp, struct md_op_data *op_data,
		    struct lookup_intent *it, struct ptlrpc_request **reqp,
		    ldlm_blocking_callback cb_blocking,
		    __u64 extra_lock_flags)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	rc = lmv_intent_lock_lmv(exp, op_data, it, reqp, cb_blocking, extra_lock_flags);
	ktget(&localclock[1]);
	ktput(localclock, lmv_intent_lock);
