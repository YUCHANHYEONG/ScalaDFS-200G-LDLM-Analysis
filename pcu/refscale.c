...
//
// Scalability test comparing PCU vs other mechanisms
...
MODULE_PARM_DESC(scale_type, "Type of test (pcu, spcu, refcnt, rwsem, rwlock.");

torture_param(int, verbose, 0, "Enable verbose debugging printk()s");
torture_param(int, verbose_batched, 0, "Batch verbose debugging printk()s");

// Wait until there are multiple CPUs before starting test.
torture_param(int, holdoff, IS_BUILTIN(CONFIG_PCU_REF_SCALE_TEST) ? 10 : 0,
...
static void ref_pcu_read_section(const int nloops)
{
	int i;
...
static void ref_pcu_delay_section(const int nloops, const int udl, const int ndl)
{
	int i;
...
static struct ref_scale_ops pcu_ops = {
	.init		= pcu_sync_scale_init,
	.readsection	= ref_pcu_read_section,
	.delaysection	= ref_pcu_delay_section,
	.name		= "pcu"
};

// Definitions for SPCU ref scale testing.
DEFINE_STATIC_SPCU(spcu_refctl_scale);
static struct spcu_struct *spcu_ctlp = &spcu_refctl_scale;

static void spcu_ref_scale_read_section(const int nloops)
{
	int i;
	int idx;

	for (i = nloops; i >= 0; i--) {
		idx = spcu_read_lock(spcu_ctlp);
		spcu_read_unlock(spcu_ctlp, idx);
	}
}

static void spcu_ref_scale_delay_section(const int nloops, const int udl, const int ndl)
{
	int i;
	int idx;
...
static struct ref_scale_ops spcu_ops = {
	.init		= pcu_sync_scale_init,
	.readsection	= spcu_ref_scale_read_section,
	.delaysection	= spcu_ref_scale_delay_section,
	.name		= "spcu"
};

// Definitions for PCU Tasks ref scale testing: Empty read markers.
// These definitions also work for PCU Rude readers.
static void pcu_tasks_ref_scale_read_section(const int nloops)
{
	int i;
...
static void pcu_tasks_ref_scale_delay_section(const int nloops, const int udl, const int ndl)
{
	int i;
...
static struct ref_scale_ops pcu_tasks_ops = {
	.init		= pcu_sync_scale_init,
	.readsection	= pcu_tasks_ref_scale_read_section,
	.delaysection	= pcu_tasks_ref_scale_delay_section,
	.name		= "pcu-tasks"
};

// Definitions for PCU Tasks Trace ref scale testing.
static void pcu_trace_ref_scale_read_section(const int nloops)
{
	int i;
...
static void pcu_trace_ref_scale_delay_section(const int nloops, const int udl, const int ndl)
{
	int i;
...
static struct ref_scale_ops pcu_trace_ops = {
	.init		= pcu_sync_scale_init,
	.readsection	= pcu_trace_ref_scale_read_section,
	.delaysection	= pcu_trace_ref_scale_delay_section,
	.name		= "pcu-trace"
};
...
static void pcu_scale_one_reader(void)
{
	if (readdelay <= 0)
...
// Reader kthread.  Repeatedly does empty PCU read-side
...
repeat:
	...
	pcu_scale_one_reader();
	if (!atomic_dec_return(&n_warmedup))
		while (atomic_read_acquire(&n_warmedup))
			pcu_scale_one_reader();
	// Also keep interrupts disabled.  This also has the effect
	// of preventing entries into slow path for pcu_read_unlock().
...
ref_scale_init(void)
{
	...
	static struct ref_scale_ops *scale_ops[] = {
		&pcu_ops, &spcu_ops, &pcu_trace_ops, &pcu_tasks_ops, &refcnt_ops, &rwlock_ops,
...
unwind:
	...
	ref_scale_cleanup();
	if (shutdown) {
		WARN_ON(!IS_MODULE(CONFIG_PCU_REF_SCALE_TEST));
		kernel_power_off();
	}
	return firsterr;
}
